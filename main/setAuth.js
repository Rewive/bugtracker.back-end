...
module.exports = async (req, res) => {
    ...
    if (await db.UserGet({ login })) return res.status(400).json({errors: [{message: "Bad request", param: "login", msg: "Данный login уже используется"}]});
    const password = await argon2.hash(req.body.password);
    const token = jwt.sign({ id }, secret, { expiresIn: '30d' });
    await db.UserSet({ _id: id }, { $set: { login, password, token } });
